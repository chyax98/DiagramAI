...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!username || !password) {
      return NextResponse.json({ error: "缺少必填字段: username 或 password" }, { status: 400 });
    }
    ...
    if (username.length > 20) {
      return NextResponse.json({ error: "用户名最多 20 个字符" }, { status: 400 });
    }

    // 验证用户名只包含字母、数字、下划线
    if (!/^[a-zA-Z0-9_]+$/.test(username)) {
      return NextResponse.json({ error: "用户名只能包含字母、数字、下划线" }, { status: 400 });
    }
