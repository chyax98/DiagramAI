/**
 * L3: Vega-Lite Line Chart 生成提示词
 *
 * 作用：定义折线图的生成规则、示例和最佳实践
 * Token 预算：800-1200 tokens
 * 图表类型：Vega-Lite Line Chart（折线图）
 *
 * 用途：
 * - 时间序列数据（如用户增长、股票价格、温度变化）
 * - 趋势分析（如销售趋势、流量变化）
 * - 多系列对比（如不同产品的增长曲线）
 * - 连续数据展示
 *
 * @example
 * 用户输入："2024年1-6月的用户增长趋势，分别是 1000、1500、1800、2200、2800、3500 人"
 * 输出：完整的 Vega-Lite Line Chart JSON 代码
 */

export const VEGALITE_LINE_PROMPT = `
