...
interface AIModel {
  id: number;
  name: string;
  provider: AIProvider; // 使用数据库类型,支持所有 Provider
  api_endpoint: string;
...
// eslint-disable-next-line max-lines-per-function, complexity -- 对话框组件包含完整的表单逻辑,拆分会降低可读性
export function ModelDialog({ isOpen, onClose, onSuccess, model }: ModelDialogProps) {
  // Provider 选项从 Registry 动态获取 (使用 useMemo 缓存,避免每次渲染重新创建)
  const PROVIDER_OPTIONS = useMemo(
    () =>
      getAllProviders().map((config) => ({
        value: config.id,
  ...
  useEffect(() => {
    if (!isEditMode && selectedProvider && isOpen) {
      const defaultEndpoint = getProviderDefaultEndpoint(selectedProvider);
      if (defaultEndpoint) {
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-5">
            ...
            <div>
              ...
              >
                ...
                {!isEditMode && selectedProvider && hasDefaultEndpoint(selectedProvider) && (
              ...
              <Input
                ...
                placeholder={
                  selectedProvider && !hasDefaultEndpoint(selectedProvider)
                    ? "请输入 API 端点地址"
