 ...
 * 输出：完整的 WaveDrom Register JSON 代码
 */

export const WAVEDROM_REGISTER_PROMPT = `
# WaveDrom Register 寄存器图生成要求
...
1. **硬件架构设计专家**
   - 理解寄存器位字段的设计原则和约定
   - 识别不同类型的位字段（控制位、状态位、数据位）
   - 掌握常见指令编码和协议帧结构

2. **WaveDrom 工程师**
   - 精通 WaveJSON 的 reg 字段结构
   - 熟练使用 bits、name、attr、type 等属性
   - 掌握位字段颜色和样式定制
...
**原因**：type 值应在 2-7 范围内，超出范围可能导致渲染异常。
