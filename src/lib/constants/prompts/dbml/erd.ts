 ...
 * L3: DBML 实体关系图生成提示词
 *
 * 作用：定义实体关系图（ER Diagram）的设计规则
 * Token 预算：800-1200 tokens
 * 图表类型：DBML ERD（Entity-Relationship Diagram）
 *
 * 用途：专注于展示实体间的关系，简化字段显示，突出关系类型和基数
 *
 * @example
 * 用户输入："设计一个图书管理系统的 ER 图，展示实体关系"
 * 输出：聚焦关系的 DBML 代码
 */

import { DBML_LANGUAGE_SPEC } from "./common";

export const DBML_ERD_PROMPT = `
# DBML 实体关系图生成要求
