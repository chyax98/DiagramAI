...
export function RecommendationCard({ recommendation, onApply, onClose }: RecommendationCardProps) {
  ...
  const typeInfo = LANGUAGE_DIAGRAM_TYPES[recommendation.renderLanguage as RenderLanguage]?.find(
    (t: { value: string }) => t.value === recommendation.diagramType
  );
              ...
              {recommendation.alternatives && recommendation.alternatives.length > 0 && (
                <div className="mt-4 pt-4 border-t">
                  ...
                  {recommendation.alternatives.map((alt, i) => {
                    ...
                    const altType = LANGUAGE_DIAGRAM_TYPES[alt.language as RenderLanguage]?.find(
                      (t: { value: string }) => t.value === alt.type
                    );
                    return (
