...
// 支持任意 AI Provider，无枚举限制
// 主流: openai, anthropic, gemini, cerebras, openai-compatible
...
export interface GenerationHistory {
  ...
  prompt_l1_id: number | null; // L1 提示词 ID（用于失败分析）
  prompt_l2_id: number | null; // L2 提示词 ID（用于失败分析）
  prompt_l3_id: number | null; // L3 提示词 ID（用于失败分析）
  created_at: string;
}
...
export interface CreateHistoryParams {
  ...
  prompt_l1_id?: number | null; // 可选,L1 提示词 ID
  prompt_l2_id?: number | null; // 可选,L2 提示词 ID
  prompt_l3_id?: number | null; // 可选,L3 提示词 ID
}
