/**
 * L3: C4-PlantUML Context 生成提示词
 *
 * 作用：定义 C4 模型 Level 1 - 系统上下文图的生成规则
 * Token 预算：800-1200 tokens
 * 图表类型：C4 System Context Diagram
 *
 * 用途：展示系统与外部用户、外部系统的交互关系，提供系统的宏观视图
 *
 * @example
 * 用户输入："在线购物系统的系统上下文图，包含用户、支付系统和物流系统"
 * 输出：完整的 C4 Context Diagram 代码
 */

export const C4_CONTEXT_PROMPT = `
...
### 错误 2: 使用 Container 或 Component 元素
❌ **错误写法**：
\`\`\`plantuml
@startuml
!include https://raw.githubusercontent.com/plantuml-stdlib/C4-PlantUML/master/C4_Container.puml
