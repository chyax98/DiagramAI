...
export interface EditorActions {
  ...
  /** 导航到提示词管理页面 */
  handleNavigatePrompts: () => void;
  /** 导航到失败日志页面 */
  handleNavigateFailureLogs: () => void;
}
...
export function useEditorActions(): EditorActions {
   ...
   * 导航到提示词管理页面
   */
  const handleNavigatePrompts = useCallback(() => {
    router.push("/prompts");
  }, [router]);
   ...
   * 导航到失败日志页面
   */
  const handleNavigateFailureLogs = useCallback(() => {
    router.push("/render-failures");
  }, [router]);
