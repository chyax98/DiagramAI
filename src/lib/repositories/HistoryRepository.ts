  ...
  create(params: CreateHistoryParams): number {
    const stmt = this.db.prepare(`
      INSERT INTO generation_histories
        (user_id, input_text, render_language, diagram_type, generated_code, model_id, is_saved, render_error, prompt_l1_id, prompt_l2_id, prompt_l3_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    ...
    const result = stmt.run(
      ...
      params.prompt_l1_id || null,
      params.prompt_l2_id || null,
      params.prompt_l3_id || null
    );
