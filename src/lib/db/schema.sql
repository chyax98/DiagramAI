...
CREATE TABLE IF NOT EXISTS ai_models (
  ...
  -- 'openai' | 'anthropic' | 'google' | 'gemini' | 'openai-compatible'
  -- 注意: 'gemini' 是 'google' 的别名, 'openai-compatible' 支持 DeepSeek 等
  ...
  -- 约束检查
  CHECK (length(name) > 0),
  CHECK (provider IN ('openai', 'anthropic', 'google', 'gemini', 'openai-compatible')),
...
CREATE TABLE IF NOT EXISTS generation_histories (
  ...
  -- 渲染语言: 枚举值 (字段名更新 - Migration 004, 扩展 - Migration 005)
  -- 'mermaid' | 'plantuml' | 'd2' | 'graphviz' | 'wavedrom' | 'nomnoml' | 'excalidraw'
  ...
  CHECK (render_language IN ('mermaid', 'plantuml', 'd2', 'graphviz', 'wavedrom', 'nomnoml', 'excalidraw', 'c4plantuml', 'vegalite', 'dbml')),
...
SELECT
  ...
  u.created_at,
  u.last_login_at,
  (SELECT COUNT(*) FROM ai_models WHERE user_id = u.id) AS model_count,
  (SELECT COUNT(*) FROM generation_histories WHERE user_id = u.id) AS history_count,
  (SELECT COUNT(*) FROM chat_sessions WHERE user_id = u.id) AS session_count
FROM users u;

-- 历史记录详情: 包含用户名和模型名称
CREATE VIEW IF NOT EXISTS history_details AS
SELECT
  h.id,
  h.user_id,
  u.username,
