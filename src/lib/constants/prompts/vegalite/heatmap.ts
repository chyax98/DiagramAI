/**
 * L3: Vega-Lite Heatmap 生成提示词
 *
 * 作用：定义热力图的生成规则、示例和最佳实践
 * Token 预算：800-1200 tokens
 * 图表类型：Vega-Lite Heatmap（热力图）
 *
 * 用途：
 * - 二维数据密度展示（如时间段活跃度、地理热度）
 * - 相关性矩阵可视化（如特征相关性、用户行为关联）
 * - 周期性模式展示（如每周每天的数据分布）
 * - 多维数据比较（如不同类别在不同条件下的表现）
 *
 * @example
 * 用户输入："展示一周内每天各时段的访问量热力图，周一0-8点100人，8-16点200人，16-24点150人..."
 * 输出：完整的 Vega-Lite Heatmap JSON 代码
 */

export const VEGALITE_HEATMAP_PROMPT = `
 ...
 * - 专家视角: 110 tokens
 * - 核心语法: 240 tokens
 * - 生成示例: 560 tokens（3个示例，每个约 185-190 tokens）
