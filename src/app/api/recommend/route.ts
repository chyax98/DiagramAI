...
import { isValidRenderLanguage } from "@/lib/constants/diagram-validation";
  ...
  try {
    ...
    try {
      ...
      const supportedTypes =
        LANGUAGE_DIAGRAM_TYPES[recommendation.renderLanguage as RenderLanguage];
      if (!supportedTypes) {
      ...
      const isValidType = supportedTypes.some(
        (type: { value: string }) => type.value === recommendation.diagramType
      );
      if (!isValidType) {
        logger.error("[/api/recommend] 推荐的图表类型不匹配:", {
          language: recommendation.renderLanguage,
          type: recommendation.diagramType,
          supportedTypes: supportedTypes.map((t: { value: string }) => t.value),
        });
        throw new Error(
